...
AUC <- function(x,l){
        library(ROCR)
        naIdx <- is.na(x) | is.na(l)  # null indexing
        # omitting null values
        x <- x[!naIdx] 
        l <- l[!naIdx]
        # performance measure for the evaluation 
...
X.sparse.train=as(X.sparse.train, "dgCMatrix") # type of matrix, coverting to dgCMatrix
...
# the actual result of train and test dataset
